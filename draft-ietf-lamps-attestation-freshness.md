...
~~~
 ...
 NonceResponse ::= SEQUENCE {
    ...
    -- the nonce valid
    type ATTESTATION-STATEMENT.&id({AttestationStatementSet}) OPTIONAL,
    -- indicates which attestation statement type the nonce applies to
...
{{I-D.ietf-lamps-csr-attestation}}. The
...
statement with the corresponding nonce used by the Attester. If `reqInfo`
or `respInfo` is present, its syntax and semantics are defined by the
selected type. The response MAY return type-specific information derived
from the request or from the selected attestation instance. For example,
a TPM-specific type could carry the PCR selection and certificate-name
values used for a challenge-response exchange such as the TPM procedures
described in {{RFC9684}}.
...
Evidence from the respective Attester, as optionally indicated by type. If
`respInfo` is present, the end entity MUST apply it according to the
selected type when requesting fresh attestation information. If a nonce is
provided in a NonceResponse without indicating any type, it can be used for
any attestation statement requiring a nonce.
...
request to aid correlation. The optional "respInfo" member contains
...
dotted-decimal form. The optional "reqInfo" member contains
...
if present, is a text string containing an `AttestationStatement` OID in
dotted-decimal form. The optional "respInfo" member contains
